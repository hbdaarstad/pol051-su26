...
terrorism_data = tibble(
 terrorist_attack = rnorm(n = , mean = , sd = ,),
 anxiety = rnorm(n = , mean = , sd = ,),
 ext_cand = rnorm(n = , mean = , sd = ,)
)
...
terrorism_data = tibble(
 education = rnorm(n = , mean = , sd = ,),
 income = sample(c(), size = , replace = TRUE),
 ideology = rnorm(n = , mean = , sd = ,)
)
...
sample_police = therm |> rep_sample_n(size = , reps = , replace = TRUE) |>
...
therm |>
  rep_sample_n(size = nrow(), reps = , replace = TRUE) |>
   summarise(avg_ = mean(, na.rm = TRUE)) |>
   summarise(low = quantile(, .025), # middle 95% means lower bound is .025
            mean = mean(, na.rm = TRUE), 
            high = quantile(, .975)) # middle 95% means upper bound is .975
